...
\begin{itemize}
  ...
  \item Turn on the microcontroller on and issue the \verb|calibrate| (see
...
\begin{description}
  ...
  \item[Rear wheel and steer angles and rates] Units of rad, rad/s. Rear wheel
    ...
    forward speeds (and by extension rear wheel rates) we constants, so the
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \addplot[red] table[
      x=time,
      y=v
    ]{data/000-time-hw_button-steer-steer_est-v-v_c-20.00-30.00-decimated.txt};
    \label{measured_v}
    ...
    %\addlegendimage{/pgfplots/refstyle=measured_v}\addlegendentry{Measured}
    %\addlegendimage{/pgfplots/refstyle=commanded_v}\addlegendentry{Commanded}
    \addplot[blue] table[
      x=time,
      y=steer
    ]{data/000-time-hw_button-steer-steer_est-v-v_c-20.00-30.00-decimated.txt};
  ...
  \caption[Run 000 speed and steer response.]{Run 000, measured speed
    ...
    released at $t$ = 21.56s, shortly before state estimation began at $t$ =
    21.62s (\ref{measured_v} $v$ = 1 m / s). See \ref{rb:subsec:sensors} for a
    description of this switch.}
  \label{rb:fig:run000a}
\end{figure}

Estimates of steer rate $\dot{\hat{\delta}}$ and lean rate $\dot{\phi}$ are
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[xlabel=Time (s),
                 ylabel=Lean rate (rad / s),
                 axis x line*=bottom,
                 axis y line*=left,
                 legend pos=outer north east]
    \addplot table[
      x=time,
      y=gyro_x]{data/000-time-gyro_x-lean_rate_est-20.00-30.00-decimated.txt};
    \addlegendentry{Measured}
    \addplot table[
      x=time,
      y=lean_rate_est]{data/000-time-gyro_x-lean_rate_est-20.00-30.00-decimated.txt};
    \addlegendentry{Estimated}
    \end{axis}
  \end{tikzpicture}
  \caption[Run 000 lean rate measured and estimated.]{Run 000 steer rate
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \addplot table[
      x=time,
      y=steer_rate]{data/000-time-steer_rate-steer_rate_est-20.00-30.00-decimated.txt};
    \addlegendentry{Measured}
    \addplot table[
      x=time,
      y=steer_rate_est]{data/000-time-steer_rate-steer_rate_est-20.00-30.00-decimated.txt};
    \addlegendentry{Estimated}
    \end{axis}
  \end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      axis x line*=bottom,
      xlabel=Time (s),
      ylabel=Rear wheel torque (N m),
      y axis style=red!75!black
    ]
    \addplot[red] table[
      x=time,
      y=T_rw
    ]{data/000-time-T_rw-T_rw_desired-T_s-T_s_desired-20.00-30.00-decimated.txt};
    \label{rb:fig:T_rw}
    \addplot[red,dashed] table[
      x=time,
      y=T_rw_desired
  ...
  \caption[Run 000 rear wheel and steer torque response.]{Run 000 rear wheel
    ...
    [20.88s, 21.57s]). Steer torque \ref{rb:fig:T_s} did not saturate. The
...
The frequency content of the measured signals gives an indication of the
dynamics that are excited. The transfer function from the applied steer torque
to the measured signals (rate gyroscope, accelerometer, steer angle encoder)
can be computed by taking the discrete Fourier transform (DFT) of the measured
signals and dividing it by the DFT of the applied input. Since the dynamics of
the bicycle are speed dependent, the FFT should be computed on a portion of the
time response which has nearly constant speed.
...
dynamics are speed dependent, the select input and output should be time series
...
\begin{align} X_k &=
  ...
  \label{rb:dft}
\end{align}
This output sequence possesses Hermitian symmetry ($X_k = X_{n-k}^*$), so half
of the output is redundant (the second half is the complex conjugate of the
first half, in reverse order). Therefor, the result of the DFT of a sequence
of $n$ real numbers is a sequence of $n/2 + 1$ complex numbers, where the
division by 2 is rounded down. The first element of this output sequence, $X_0$
is always real and represents $n$ times the mean of the input sequence $x_k$
(i.e., $E\{x_k\} = n X_0$). The magnitude of the remaining $n/2$ complex
numbers can be interpreted $\frac{1}{2}$ the magnitude of the frequency
response, at frequencies evenly spaced between 0 and the Nyquist frequency
($f_s / 2$). In practice, to obtain the magnitude portion of the frequency
response, one needs to compute $\frac{2}{N} |X_k|$ for $k = 1 \ldots n/2$, and
$\frac{1}{N} |X_0|$ for the DC component. However, to estimate a transfer
function given the DFT of the input and output of a linear system, only the
ratio of the DFTs need to be considered, so this scaling is unnecessary (it can
be performed on each signal but the effect will cancelled out when computing
the ratio).

Estimates of $|\frac{\delta}{\tau_\delta}(j\omega)|$ and
$|\frac{\dot{\phi}}{\tau_\delta}(j\omega)|$ were computed for three different
speeds: 1, 2, and 3 m/s. The time series data was selected over a time interval
during which the forward speed of the bicycle was nearly constant. Since the
sampling frequency was 200Hz, all frequency responses are plotted up to 100Hz
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption[Steer, lean rate, steer torque time response at 1.0 m / s.]{Run 024 steer,
  lean rate, steer torque and speed response to $v_c=1.0$ m / s, $\mu_v=0.959$
m / s, $\sigma_v=0.0157$ m / s. The run continued until $t$ = 41s with very
similar response.}
  \label{rb:fig:run000_tr}
\end{figure}
\begin{figure}[htbp]
  \pgfplotstableread{data/024-freq-T_s-gyro_x-steer-v-21.00-41.00-fft_decimated.txt}\runtwentyfourfreq
  \centering
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption[Steer and lean rate frequency response to $v_c=1.0$ m / s.]{Run 024
  steer and lean rate frequency response, $v_c=1.0$ m / s, $\mu_v=0.959$ m / s,
  $\sigma_v=0.0157$ m / s.  DFT of steer, lean rate, and steer torque signals was
  computed for $t \in$ [21s, 41s] (4001 time points).}
  \label{rb:fig:run000_fr}
\end{figure}
