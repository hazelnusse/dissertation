...
set point to \SI{1.0}{\m\per\s}, and optionally, an additive sinusoidal disturbance steer
torque of magnitude $a_d$ (\SI{1e-2}{\N\m}) and frequency $f_d$ (\si{\Hz}). Dr. Hubbard took notes
...
less than \SI{0.01}{\m\per\s\squared} and 1.0 deg, respectively), then ran along side the
...
set point to \SI{1.0}{\m\per\s}. At \SI{1.0}{\m\per\s}, it was easy to manually switch off the motors
...
\begin{figure}[htbp]
  ...
  \caption[Steer, lean rate, steer torque time response at
    ...
    response to $v_c=\SI{1.0}{\m\per\s}$. The run continued until $t=\SI{41}{\s}$
...
\begin{figure}[htbp]
  ...
  \caption[Steer and lean rate frequency response to
    $v_c=\SI{1.0}{\m\per\s}$.]{Run 024 steer and lean rate frequency response,
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption[Steer, lean rate, steer torque time response at 4.0 m / s.]{Run 025 steer,
    lean rate, steer torque and speed response to $v_c=\SI{2.0}{\m\per\s}$.}
...
\begin{figure}[htbp]
  ...
  \caption[Steer, lean rate, steer torque time response at \SI{4.0}{\m\per\s}.]{Run 007 steer,
    lean rate, steer torque and speed response to $v_c=\SI{4.0}{\m\per\s}$.}
