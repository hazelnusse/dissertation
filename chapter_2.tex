...
ground plane. The frame and fork are each assumed to to be inertially inertial
symmetric about in their respective $XZ$ planes. The revolute joint connecting
the bicycle frame and fork is assumed to be parallel to the $Z$ axis of the
bicycle frame and fork. It is assumed that each wheel mass center lies in the
...
The choice of physical parameters used to describe the four rigid bodies, their
interface with each other and the ground plane is of practical concern for
several reasons: 1) measurement of the parameters on real bicycles, 2)
simulation efficiency, 3) comparisons between models. Measurement of physical
bicycle parameters is laborious and it is therefore beneficial to have a set of
model parameters that are easily (and ideally directly) measurable on standard
bicycles. The parameters used in~\cite{Meijaard2007} (this is not the first use
of this parameter set, but it is the most clearly presented; hereafter we refer
to these as the ``Meijaard parameters'') have been adopted by many
authors~\cite{Sharp2008,Moore2010} as the \textit{de facto} set of parameters
for describing bicycles. Further, many of the Meijaard parameters are
immediately recognizable by everyday bicycle enthusiasts, for example wheelbase
$w$, trail $c$, and steer axis tilt $\lambda$. For these reasons, this choice
of parameters is indispensable.
...
\begin{table}[ht]
  ...
  axis separation $l_s$ is a property of how the two gyrostats are connect and
