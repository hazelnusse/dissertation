...
The other custom fabricated mechanical components were the wheel optical
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    \node (disturbance) [above=of inner_sum] {$d = A\sin(2\pi f t)$};
...
\begin{description}
  ...
  \item[\Q{calibrate}] Begin the steer angle calibration routine.
  \item[\Q{homefork}] Wait for the steer encoder index signal set the steer calibration offset.
  \item[\Q{e_thresh {+-}XX.XX}] Set state estimation threshold speed in m/s.
  ...
  \item[\Q{c_thresh {+-}XX.XX}] Set control threshold speed in m/s. Must be larger than estimation threshold speed.
  \item[\Q{thresh {+-}XX.XX {+-}XX.XX}] Simultaneously set state estimation
    and control threshold speeds in m/s.
  \item[\Q{disturb {+-}XX.XX {+-}XX.XX}] Set sinusoidal disturbance amplitude (in
    cNm) and frequency (in Hz).
  \item[\Q{speed_limit {+-}XX.XX {+-}XX.XX}] Set reference speed (in m/s) and
  ...
  \item[\Q{speed {+-}XX.XX}] Set reference speed (in m/s). The bicycle will
    immediately attempt to track this speed until a fault occurs or the
    reference is changed by the user.
  \item[\Q{l_thresh}] Set lateral acceleration threshold for lean indication
  ...
  \item[\Q{yaw_rate {+-}XX.XX}] Set the reference yaw rate (in rad/s). On
    reset, reference yaw rate is 0 rad/s (straight line motion).

\end{description}

%\section{Experiments and results} \label{rb:sec:experiments}
