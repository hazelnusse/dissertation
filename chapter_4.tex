...
first half, in reverse order). Therefore, the result of the DFT of a sequence of
$n$ real numbers is a sequence of $n/2 + 1$ complex numbers, where the division
by 2 is rounded down. The first element of this output sequence, $X_0$ is
always real and represents $n$ times the mean of the input sequence $x_k$
(i.e., $E\{x_k\} = n X_0$). The magnitude of each of the remaining
$\frac{n}{2}$ complex numbers is $\frac{1}{2}$ the magnitude of the frequency
...
ratio of the DFTs need to be considered, so this scaling is unnecessary.

Estimates of $|\frac{\delta}{\tau_\delta}(j\omega)|$ and
$|\frac{\dot{\phi}}{\tau_\delta}(j\omega)|$ were computed for three different
commanded speeds: 1, 2, and 4 m / s. The time series data was selected over a
time interval during which the forward speed of the bicycle was nearly
constant. Since the sampling frequency was 200Hz, all frequency responses are
plotted up to 100Hz (the Nyquist frequency).
\begin{figure}[htbp]
  ...
  similar response. Steer torque saturated repeatedly at
  $\tau_{\delta,max}=\pm4.511$ N m.}
...
\end{figure}
Control of bicycles is more difficult at low speeds. This fact manifests itself
as large state feedback gains which increase noise sensitivity.  This effect is
clearly apparent in \autoref{rb:fig:run024_tr} where high frequency, large
magnitude (saturating) steer torque oscillations were observed.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption[Steer, lean rate, steer torque time response at 4.0 m / s.]{Run 007 steer,
  lean rate, steer torque and speed response to $v_c=4.0$ m / s, $\mu_v=3.949$
  m / s, $\sigma_v=0.0225$ m / s.}
  \label{rb:fig:run007_tr}
\end{figure}
\begin{figure}[htbp]
  \pgfplotstableread{data/007-freq-T_s-gyro_x-steer-v-21.00-31.00-fft_decimated.txt}\runsevenfreq
  \centering
  \begin{tikzpicture}
    \begin{axis}[xlabel=Frequency (Hz),
      ylabel=$|\frac{\delta}{\tau_\delta}|$,
    ...
    \end{axis}
  \end{tikzpicture}
  \caption[Steer and lean rate frequency response to $v_c=4.0$ m / s.]{Run 024
  steer and lean rate frequency response, $v_c=4.0$ m / s, $\mu_v=3.949$ m / s,
  $\sigma_v=0.0225$ m / s.  DFT of steer, lean rate, and steer torque signals was
  computed for $t \in$ [21s, 31s] (2001 time points).}
